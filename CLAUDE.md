...
- `renderActions()` / `weeksUntil(type)` : 관찰 상단 **"이번 주 할 일"** 액션 대시보드. 현재 상태(위기군 비율·소그룹 밖 외톨이·비신앙 가정 위기·부모 다락방)와 다가오는 일정(수능·시험 임박, 중3→고1 전환 임박)을 훑어 시급(높음/중간/낮음)순으로 "지금 무엇이 필요한가 → 그래서 무엇을"을 띄운다. 목사·갱신팀이 한눈에 손볼 곳을 본다.
